...
{
  /// <summary>
  /// Class implementing a inverse of low-pass & G filter.
  /// </summary>
  public class SEFilter : IIRFilterBase
  {
    ...
    protected override void CalculateIIRCoeff()
    {
      ...
      Array.Resize(ref Zeros, 3);
      Array.Resize(ref FilterStateZ, 3);
      Array.Resize(ref Poles, 3);
      Array.Resize(ref FilterStateP, 4);
    ...
    protected override int DoCheckSettings()
    {
      ...
      if (result == 0)
      {
        if ((Setting[3].Value <= 0) || (Setting[3].Value > SampleFrequency / 2))
          ...
          if ((Setting[4].Value <= 0) || (Setting[4].Value > SampleFrequency / 2))
            result = 4;
          else
            if ((Setting[5].Value <= 0) || (Setting[5].Value > SampleFrequency / 2))
              result = 5;
      }
      return result;
    }
    ...
    public SEFilter()
      : base(5)
    {
      Setting[0].Info = "Inverse of low-pass & G";
